...
// ioctl(fd, CMD, ARG)
static long
sr04_ioctl(struct file *filp, unsigned int command, unsigned long arg)
{
	// send trig
	switch (command) {
	case CMD_TRIG:
...
static irqreturn_t sr04_isr(int irq, void *dev_id)
{
	...
	else
	{
		...
		/* falling edge record the end time and cal the gap time, cal the dist*/
