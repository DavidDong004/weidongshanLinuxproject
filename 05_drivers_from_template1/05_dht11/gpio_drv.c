...
static ssize_t dht11_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	mdelay(18);
	gpio_direction_input(gpios[0].gpio); /* input + 1 */
	...
	kern_buf[0] = get_key();
	kern_buf[1] = get_key();
	printk("%s %s %d, get value : 0x%x, 0x%x\n", __FILE__, __FUNCTION__, __LINE__, kern_buf[0], kern_buf[1]);

	if ((kern_buf[0] == (char)-1) && (kern_buf[1] == (char)-1))
	{
		printk("%s %s %d, get err value\n", __FILE__, __FUNCTION__, __LINE__);
		// free_irq(gpios[0].irq, &gpios[0]);
		return -EIO;
	}
...
static ssize_t dht11_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
...
static void parse_dht11_datas(void)
{
	...
	for (i = g_dht11_irq_cnt - 80; i<84; i += 2)
	{
		...
		// if (low_time < 60000 && low_time > 40000)
		// {
...
static int __init dht11_init(void)
{
	...
	{		
		...
		err = gpio_direction_output(gpios[i].gpio, 1);
		...
		//err = request_irq(gpios[i].irq, dht11_isr, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "100ask_dht11", &gpios[i]);
	}
