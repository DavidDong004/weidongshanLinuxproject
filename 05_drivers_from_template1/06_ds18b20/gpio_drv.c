...
static void ds18b20_calac_val(unsigned char ds18b20_buf[], int result[])
{
...
static ssize_t ds18b20_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	if (err)
	{
		...
		printk("%s %s line %d ds18b20_reset_and_wait_ack err\n", __FILE__, __FUNCTION__, __LINE__);
		return err;
	}
	...
	{
		spin_unlock_irqrestore(&ds18b20_spinlock, flags);
		printk("%s %s line %d ds18b20_reset_and_wait_ack second err\n", __FILE__, __FUNCTION__, __LINE__);
		return err;
	}
	...
	if (err) 
	{
		printk("%s %s line %d ds18b20_reset_and_wait_ack crc err\n", __FILE__, __FUNCTION__, __LINE__);
		return err;
	}
	/* 4. copy_to_user */
	ds18b20_calac_val(kern_buf, result_buf);
