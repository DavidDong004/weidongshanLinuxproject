...
{	
	...
	{
		printk("%s %s line %d data verify err\n", __FILE__, __FUNCTION__, __LINE__);
		goto err;
	}
...
static int parse_irda_repeat_datas(void)
{	
	u64 time;
...
static irqreturn_t irda_isr(int irq, void *dev_id)
{
	...
	/* 1. 记录中断发生的时刻 */	
	time = ktime_get_ns();
	g_irda_irs_times[g_irda_irs_cnt] = time;
	...
	{
		/* 是否重复码 */
		if (0 == parse_irda_repeat_datas())
		{
			/*device: 0, val:0, repeat code*/
			// printk("%s %s line %d in parse_irda_repeat_datas\n", __FILE__, __FUNCTION__, __LINE__);
			put_key(0);
			put_key(0);
			wake_up_interruptible(&gpio_wait);
			kill_fasync(&button_fasync, SIGIO, POLL_IN);
