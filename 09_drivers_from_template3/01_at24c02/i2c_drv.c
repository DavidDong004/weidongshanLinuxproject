...
static ssize_t i2c_drv_read (struct file *file, char __user *buf, size_t size, loff_t *offset)
{
	...
	/* from 0 read size byte */

	kern_buf = kmalloc(size,GFP_KERNEL);

	/* 初始化i2c_msg 
	 * 1. start a write operation,; send 0 to AT24C02, show that the MPU need read data from 0 addr
	 * 2. start a read operation,; get data
	 */

	msgs[0].addr  = g_client->addr;
	msgs[0].flags = 0; /* write operation */
	msgs[0].buf   = kern_buf;
	kern_buf[0]	  = 0; /*send 0 to device*/
	msgs[0].len   = 1;

	msgs[1].addr  = g_client->addr;
	msgs[1].flags = I2C_M_RD; /* write operation */
	msgs[1].buf   = kern_buf;
	msgs[1].len   = size;
...
static ssize_t i2c_drv_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
	...
	while (size > 0)
	{
		...
		*/
		...
		msgs[0].flags = 0; /* write operation */
		msgs[0].buf   = kern_buf;
		kern_buf[0]	  = addr; /*wirte AT24C02 adrr*/
		msgs[0].len   = len+1;
		addr += len;
